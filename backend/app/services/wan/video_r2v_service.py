...
def generate_r2v(
    ...
    *,
    ...
    # Log payload at INFO level for production visibility
    logger.info(f"R2V Request Payload: {json.dumps(payload)}")
    ...
    if not video_url:
        err_msg = f"R2V: video URL not found in response: {start_data}"
        logger.error(err_msg)
        raise HTTPException(status_code=502, detail=err_msg)
    logger.info("R2V generation successful (sync)")
